...
Each gold stadard is contained in a single file in the ``gold_standard`` 
folder.  These files are in a simple tabular format where each row represents
the membership of a particular protein (given by its systematic name) in a
particular complex. Columns are separated by whitespace; the first column
 contains the ID of the protein, while the second column contains the ID of the 
complex.
